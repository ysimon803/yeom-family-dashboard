...
type EditFinancialGoalModalProps = {
  goal: FinancialGoal;
...
export default function EditFinancialGoalModal({
  goal,
  isSubmitting = false,
  onClose,
  onSubmit,
}: EditFinancialGoalModalProps) {
  const [formData, setFormData] = useState<UpdateFinancialGoalInput>(() =>
    createInitialFormState(goal),
  );
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const targetAmount = formData.target_amount ?? 0;
    ...
    try {
      ...
      console.error("Failed to update financial goal:", submitError);
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5 dark:border-slate-800">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Update the goal amount, progress, category, or target date.
            </p>
          </div>
          ...
          <button
            ...
            aria-label="Close edit financial goal modal"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="max-h-[70vh] space-y-5 overflow-y-auto px-6 py-5">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <div>
                <label
                  htmlFor="edit-goal-target-amount"
                  className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300"
                >
                  Target amount
                </label>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    disabled={isSubmitting}
                    className="w-full rounded-xl border border-slate-300 bg-white py-3 pl-8 pr-4 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
                  />
                </div>
              </div>
              ...
              <div>
                <label
                  htmlFor="edit-goal-current-amount"
                  className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300"
                >
                  Current amount
                </label>
